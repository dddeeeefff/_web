...
// 회원 가입에 관련된 쿼리 작업을 처리하는 클래스
	...
	// 사용자가 입력한 데이터들로 회원가입을 시키는 메소드
		...
		try {
			String sql = "insert into t_member_info (mi_id, mi_pw, mi_name, mi_gender, " +
					    "mi_birth, mi_phone, mi_email, mi_point) values (?, ?, ?, ?, ?, ?, ?, ?)";
			...
			if (result == 1) {
			// t_member_info 테이블에 insert 쿼리가 정상적으로 동작했으면
				sql = "insert into t_member_addr (mi_id, ma_name, ma_phone, ma_zip, ma_addr1, ma_addr2) " + 
						"values (?, ?, ?, ?, ?, ?)";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, memberInfo.getMi_id());
				pstmt.setString(2, memberAddr.getMa_name());
				pstmt.setString(3, memberAddr.getMa_phone());
				...
				if (result == 2) {
				// t_member_addr 테이블에 insert 쿼리가 정상적으로 동작했으면
