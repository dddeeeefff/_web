...
// 주문 관련된 쿼리 작업(폼, 등록, 변경)들을 모두 처리하는 클래스
	...
	// 주문처리를 하는 메소드(장바구니나 바로 구매를 통한 주문)
		...
		try {
			...
			if(oi.getOi_upoint() > 0) {		// 구매시 포인트를 사용했으면 
				...
				sql = "insert into t_member_point (mi_id, mp_su, mp_point, mp_desc, mp_detail) " + 
						" values ('" + oi.getMi_id() + "', 's', '" + pnt + "', '상품구매', '" + oi_id + "')";
				target++;	rcount += stmt.executeUpdate(sql);
			}
		...
		// 주문번호, 실제 적용된 레코드수, 적용되어야 할 레코드수를 리턴
	}
	...
	// 받아온 회원아이디와 주문번호에 해당하는 정보들을 OrderInfo형 인스턴스에 담아 리턴하는 메소드
		...
		ResultSet rs = null;
		OrderInfo oi = null;
		ArrayList<OrderDetail> detailList = new ArrayList<OrderDetail>();	// 주문의 상품들 정보
		
		try { 
			String sql = "select a.oi_name, a.oi_phone, a.oi_zip, a.oi_addr1, a.oi_addr2, a.oi_payment, a.oi_pay, " +  
					"b.od_img, b.od_name, b.od_size, b.od_cnt, b.od_price, b.pi_id, c.pi_isview " + 
					"from t_order_info a, t_order_detail b, t_product_info c " + 
					"where a.oi_id = b.oi_id and b.pi_id = c.pi_id and " + 
					"a.mi_id = '" + miid + "' and a.oi_id = '" + oiid + "' order by c.pi_id, b.od_size";
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
