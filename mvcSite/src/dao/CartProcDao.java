...
// 장바구니 관련된 쿼리 작업(목록, 등록, 수정, 삭제)들을 모두 처리하는 클래스
	...
	// 사용자가 선택한 상품을 장바구니에 담는 메소드
		...
		try {			
			...
			// 현재 로그인한 회원의 장바구니에 동일한 상품 및 옵션이 있으면 수량을 증가시킴
			if (result == 0) {
			// 동일한 상품(옵션포함)이 없으면 장바구니에 새롭게 추가
	...
	// 장바구니에서 보여줄 정보들을 ArrayList로 리턴하는 메소드
		...
		try {
			...
			" b.pi_price, b.pi_dc, c.ps_stock from t_order_cart a, " +
			" t_product_info b, t_product_stock c " + 
			" where a.pi_id = b.pi_id and a.pi_id = c.pi_id " +
			" and a.ps_idx = c.ps_idx and b.pi_isview = 'y' " +
			" and a.mi_id = '" + miid + "' ";
			stmt = conn.createStatement();
