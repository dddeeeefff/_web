...
// 상품 관련된 쿼리 작업(목록, 상세보기)들을 모두 처리하는 클래스
	...
	// 사용자가 선택한 상품의 정보를 ProductInfo형 인스턴스로 리턴하는 메소드
		...
		try { 
			String sql = "select a.pi_id, a.pi_name, a.pi_min, a.pi_dc, a.pi_img1, a.pi_img2, b.po_color, b.po_rank, b.po_memory, b.po_inc " + 
					" from t_product_info a, t_product_option b where a.pi_id = b.pi_id and a.pi_id = '" + piid + "' ";
			...
			if (rs.next()) {
				...
				pi.setPo_color(rs.getString("po_color"));
				pi.setPo_rank(rs.getString("po_rank"));
				pi.setPo_memory(rs.getString("po_memory"));
				pi.setPo_inc(rs.getInt("po_inc"));
			}
	...
	public ArrayList<ProductInfo> getProductList(int cpage, int psize, String where, String orderBy) {
		...
		try {
			String sql = "select a.pi_id, b.pb_name, a.pi_name, a.pi_min, a.pi_dc, a.pi_img1 from t_product_info a, t_product_brand b " +  
					" where a.pb_id = b.pb_id and a.pi_isview = 'y' " + where + " group by a.pi_id " + orderBy + " limit " + ((cpage - 1) * psize) + ", " + psize;
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
